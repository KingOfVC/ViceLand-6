class CCmdGang
{
	...
	constructor( instance )
	{
		...
		this.Create					= this.Cmd.Create( "acreategang", "s|s|g", [ "Founder", "Tag", "Name" ], 3, 3, -1, true, true );
		this.Setting				= this.Cmd.Create( "gangsetting", "s|s|s|g", [ "Type", "Value", "G", "B" ], 1, 4, -1, true, true );
		...
		this.Info					= this.Cmd.Create( "ganginfo", "g", [ "Gang" ], 0, 1, -1, true, true );
		this.Viewrank				= this.Cmd.Create( "gangviewrank", "g", [ "Gang" ], 0, 1, -1, true, true );
		...
		this.Ganglist				= this.Cmd.Create( "ganglist", "", [ "" ], 0, 0, -1, true, true );
		this.Create2				= this.Cmd.Create( "creategang", "s|g", [ "Tag", "Name" ], 2, 2, -1, true, true );
		...
		this.Ganglist.BindExec( this.Ganglist, this.GangList );		
		this.Create2.BindExec( this.Create2, this.CreateGang2 );
	}
	...
	function funcFailCommand( type, msg, payload )
	{
		...
		switch( type )
		{
			...
			switch( cmd )
			{
				...
				case "creategang":
				player.Msg( TextColor.Error, Lang.GangCreategangSyntax2[ player.Data.Language ] );
				break;
			}
		}
	}
					...
					if( !SqGang.FindGang( args.Name ) )
					{
						if( SqMath.IsGreaterEqual( args.Tag.len(), 1 ) )
						{
							if( SqMath.IsLess( args.Tag.len(), 5 ) )
							{
								...
								if( target )
								{
									if( target.Data.IsReg )
									{
										if( target.Data.Logged )
										{
											...
											SqCast.MsgAllExp( target, TextColor.Info, Lang.AGangFoundAll, target.Name, TextColor.Info, args.Name );

											target.Msg( TextColor.Sucess, Lang.GangCeateGang[ player.Data.Language ], target.Name );

											SqGang.playerJoin( target );
										}
								...
								{
			                        if( SqAccount.FindAccountByName( args.Founder ) )
			                        {
										SqGang.Create( args.Name, args.Tag, SqAccount.GetIDFromName( args.Founder ) );

										SqCast.MsgAll( TextColor.Info, Lang.AGangFoundAll, args.Founder, TextColor.Info, args.Name );
			                        }
						...
						if( SqGang.GetPlayerLevel( getID, player.Data.AccID ) != "N/A" )
						{
							if( SqMath.IsGreaterEqual( SqGang.GetPlayerLevel( getID, player.Data.AccID ), getGang.Permissions.editsetting.tointeger() ) )
							{
								switch( args.Type )
								{
									...
									if( args.rawin( "Value" ) )
									{
										local g = ( args.rawin( "G" ) ) ? args.G : "";
										local b = ( args.rawin( "B" ) ) ? args.B : "";

										SqGang.Gangs[ getID ].Description = args.Value + " " + g + " " + b
										SqGang.Save( getID );

										SqCast.MsgGang( getID, TextColor.Gang, Lang.GangAnnDespChang, player.Name, TextColor.Gang, args.Value + " " + g + " " + b );
									}
									...
									if( args.rawin( "Value" ) && args.rawin( "G" ) && args.rawin( "B" ) )
									{
										...
										{
											if( SqInteger.IsNum( args.B ) )
											{
												SqGang.Gangs[ getID ].Color = Color3( args.Value.tointeger(), args.G.tointeger(), args.B.tointeger() );
												SqGang.Save( getID );
												...
												{
													if( target.Data.ActiveGang == getID ) target.Color = Color3( args.Value.tointeger(), args.G.tointeger(), args.B.tointeger() );
												});
											}
			...
			if( player.Data.Logged )
			{
				if( player.Data.ActiveGang != "N/A" )
				{
					if( SqGang.Gangs.rawin( player.Data.ActiveGang ) )
					{
						...
						if( SqGang.GetPlayerLevel( getID, player.Data.AccID ) != null )
						{
				...
				if( player.Data.ActiveGang != "N/A" )
				{
					if( SqGang.Gangs.rawin( player.Data.ActiveGang ) )
					{
						...
						if( getGang.Founder != player.Data.AccID )
						{
							if( SqGang.GetPlayerLevel( getID, player.Data.AccID ) != "N/A" )
							{
								SqGang.Gangs[ getID ].Members.rawdelete( player.Data.AccID.tostring() );
								SqGang.Save( getID );
			...
			if( player.Data.Logged )
			{
				...
				{	
					if( SqGang.FindGang( args.Gang ) )
					{
						...
						{
							local getMembers = {};
							local t, ta , j , k = 0, i = 1, getStr = null;
			...
			if( player.Data.Logged )
			{
				if( player.Data.ActiveGang != "N/A" )
				{
					...
					else 
					{
						...
						else player.Msg( TextColor.Error, Lang.gangviewrankSyntax[ player.Data.Language ] );
					}
				}
				else player.Msg( TextColor.Error, Lang.GangNoInActiveGang[ player.Data.Language ] );
			}
					...
					if( SqGang.FindGang( args.Gang ) )
					{
						...
						if( !SqGang.FindPlayer( getID, player.Data.AccID ) )
						{
							switch( SqGang.Gangs[ getID ].IsOpen )
							{
								...
								{
									SqGang.EditPlayer( getID, player.Data.AccID, player.Data.GangInvite[ getID ].Rank );
	...
	function GangList( player, command )
	{
		if( player.Data.IsReg )
		{
			if( player.Data.Logged )
			{
				...
				player.StreamInt( 105 );
				player.StreamString( "Gang list$" + player.World );
				player.FlushStream( true );

				foreach( index, value in SqGang.Gangs )
				{
					player.StreamInt( 107 );
					player.StreamString( HexColour.Red + "Name " + HexColour.White + value.Name + HexColour.Red + " Founder " + HexColour.White + SqAccount.GetNameFromID( value.Founder ) + HexColour.Red + " Recruit type " + HexColour.White + value.IsOpen );
					player.FlushStream( true );
				}
	...
	function CreateGang2( player, args )
	{
		if( player.Data.IsReg )
		{
			if( player.Data.Logged )
			{
				if( player.Data.ActiveGang == "N/A" )
				{
					if( player.Data.GetInventoryItem( "gangpass" ) )
					{					
						if( !SqGang.FindGang( args.Name ) )
						{
							if( SqMath.IsGreaterEqual( args.Tag.len(), 1 ) )
							{
								if( SqMath.IsLess( args.Tag.len(), 5 ) )
								{
									local decItem = player.Data.Player.Inventory[ "gangpass" ].Quatity.tointeger();
									
									SqGang.Create( args.Name, args.Tag, player.Data.AccID );

									player.Data.Player.ActiveGang = SqGang.FindGang( args.Name );

									SqCast.MsgAllExp( player, TextColor.Info, Lang.AGangFoundAll, player.Name, TextColor.Info, args.Name );

									player.Msg( TextColor.Sucess, Lang.GangCeateGang[ player.Data.Language ], args.Name );

									if( player.Data.Player.CustomiseMsg.Type.HelpMsg == "true" ) player.Msg( TextColor.InfoS, Lang.CreateGangHelp[ player.Data.Language ], TextColor.InfoS );

									player.Data.Player.Inventory[ "gangpass" ].Quatity = ( decItem - 1 );

									SqGang.playerJoin( player );
								}
								else player.Msg( TextColor.Error, Lang.GangCreateTagMoreThan4[ player.Data.Language ] );
							}
							else player.Msg( TextColor.Error, Lang.GangCreateTagLessThan1[ player.Data.Language ] );
